...
namespace MinifyERPBilling.Forms.Masters
{
    public partial class JournalVoucher : Form
    {
        ...
        private bool Save()
        {
            TransactionVouchermodel transaction = new TransactionVouchermodel();
            transaction.Action = "JournalVoucherInsert";
            ...
            transaction.AccountHeadId = txt_fromAccnt.Text;
            transaction.TranscationDate = DateTime.Parse(dateTimePicker.Text);
            transaction.xmlStr = ConvertDatatableToXML(addData);
            MessageBox.Show(transaction.xmlStr);
            DataSet ds = transaction.TransactionOperation();
